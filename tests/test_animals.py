...
def test_alternation_prefers_opposite_but_never_stalls(env):
    """After a meme it prefers an animal, but if none exist it still posts a
    meme rather than going silent (no deadlock)."""
    ...
    settings_store.update({"alternate_meme_animal": True,
    ...
    # preferred = animal, none available -> must still post the meme (not None)
    picked = scheduler._pick_candidate()
    assert picked is not None and picked.subject == "memes"
